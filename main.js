...
/* Persist contact name with localStorage */
(function(){
  const nameInput = document.getElementById('name');
  if (!nameInput) return;
  const saved = localStorage.getItem('bs-name');
  if (saved) nameInput.value = saved;
  nameInput.addEventListener('input', () => localStorage.setItem('bs-name', nameInput.value));
})();

/* Simple visit cookie */
(function(){
  if (!document.cookie.split('; ').some(row => row.startsWith('visited='))) {
...
if (burger) {
  burger.addEventListener('click', () => {
    ...
    burger.setAttribute('aria-expanded', String(m.classList.contains('open')));
  });
}
